...
- 登录与权限：账号密码登录，区分管理员与普通用户两级角色；所有页面均可访问，地图操作、查询/设置编辑与数据导入限管理员。
...
- 药品下单：选择药品或扫码下单，最多保留 10 单（当前单 1 单、等待单最多 9 单）；等待单创建后显示“已排队”，当前单经 Broker 确认结束后自动切换。队列随状态文件保存，保留重复商品和待人工确认拦截，并查看工单状态和结构化失败详情。
...
- 机械臂：登录用户通过工具页面访问 `192.168.11.18` 的示教器，支持刷新。浏览器只需能访问工具服务；工具主机仍须能访问机械臂的 80、8090、8060 端口。网页、接口和实时连接均由工具转发。
- 文件管理：左右卡片分别展示独立操作的宿主机文件目录与交互终端；目录互不跟随，断开后清空终端。Docker 部署通过本机连接进程访问宿主机，使用启动部署脚本的系统账号。登录用户可预览文本与图片、上传文件/目录、下载文件/目录 ZIP；可连接已部署的浏览器远程桌面。
...
| 普通用户 | `nvidia / nvidia` | 可访问全部页面；禁止下列操作 |

普通用户禁止的操作：

文件管理、终端、远程桌面和机械臂页面向两类账号开放，无需先加载业务数据。机械臂仍使用自身账号登录。以下限制由工具页面及对应接口实施；已开放的终端按宿主机系统账号权限执行命令。

1. 库位的编辑保存（数据查询页「编辑/保存」）；
2. 设置页编辑和配置保存（含工作模式切换、下单接口、虚拟键盘、ETM、飞书表单）；查看、刷新和获取 Token 可用；
3. 数据加载页的「导入」方式（本机路径与包加载不受限）；
4. 地图导航的写操作，包括连接配置、导航/巡逻、建图、手动控制、地图修改与异常清除；查看、缩放、刷新和导出可用。

仪表板确认、自动确认、下单、测试下单、订单操作及日志服务控制按原业务规则可用。管理员不受角色限制，但仍需满足生产模式和设备状态等业务条件。
...
| 交付物 | 内容 | 更新时机 |
| --- | --- | --- |
| `knowledge_shelf_query_runtime:v1.2.0` | ARM64 Python 3.12、FastAPI/Uvicorn、WebSocket/上传依赖、Pillow、Docker CLI、Docker Compose | 运行环境变化时 |
| `knowledge_shelf_query_<版本>.bin` | Python 源码、页面模板和静态资源 | 源码变化时 |

日常更新只替换 `.bin` 文件，无需重新构建或拉取运行镜像。

本次 FastAPI 迁移必须同时升级运行镜像至 `v1.2.0`，并同步完整部署包中的启动脚本和 Compose 配置。旧运行镜像缺少新增依赖，不能只替换 `.bin`。镜像需要先构建并发布或离线导入设备，本地修改默认标签不会自动发布镜像。

代码按接口、业务、存储与外部服务访问分层，目录和依赖方向见 [架构与验证说明](docs/architecture.md)。HTTP 地址和响应格式保持兼容，应用固定单进程运行。
...
网页服务以 Python 3.12 为部署基线，依赖版本固定在 `requirements.txt`；上传解析已移除对标准库 `cgi` 的依赖。
...
终端文本直接显示在访问设备浏览器内。Docker 部署首次使用图形程序时，在宿主机部署目录执行 `bash start.sh desktop install`、`bash start.sh restart`，然后重新连接网页终端。程序窗口显示在页面“远程桌面”中，使用同一 IP、端口和当前登录会话；宿主机无需连接显示器。最小化窗口显示在桌面底部任务栏，也可用 `Alt+Tab` 恢复。安装命令仅支持 Debian/Ubuntu，会安装 TigerVNC、noVNC 等系统组件。已有独立桌面服务仍可通过 `KSQ_DESKTOP_URL` 配置入口。详见 [文件管理与显示转发](docs/manual/部署操作文档.md#十文件管理与显示转发)。

服务自身的运行日志独立写入 `logs/knowledge_shelf_query.log`，同时输出到标准错误。源码和部署包均可执行 `bash start.sh runtime-logs` 查看最近日志并持续跟随。单文件按 5 MiB 轮转，保留当前文件及 3 个历史文件，合计约 20 MiB。成功的 GET 请求（包括仪表板、健康检查、地图和终端轮询）记为 DEBUG，默认不输出；写请求、客户端错误和未处理异常仍记录。它与页面「日志查询」中的机器人服务日志不是同一份日志。

两份 Compose 配置均限制 Docker 容器日志为每份 5 MiB、最多 3 份，合计约 15 MiB。已有部署需同步新版 `docker-compose.yml` 并重建容器后生效，仅重启原容器或替换 `.bin` 不会更新 Docker 日志限制。
...
库位表也支持 `goods-locations.csv`。新版可增加可选列 `customer_location_code`：下单时原样用作 Broker 的 `items[].location_code`（保留前导零和连字符）；缺列或为空时沿用物理库位去连字符的旧逻辑。界面库位和选品仍使用 `shelf_number/level/bin_unit`。更新库位表后需重新加载数据，并重新生成或导入测试清单。
...
`KNOWLEDGE_DIR` 指向宿主机的 `model/templates` 根目录，标准容器将其挂载为 `/data/templates`；默认实际读取 `/data/templates/knowledge`。页面中的 Knowledge 路径以该根目录为基准，可填写 `knowledge`、场景目录 `pnp_percept/templates_260827` 或其 `.../knowledge` 子目录；填写场景目录时会自动定位 `knowledge` 子目录，不再依赖 VfmApp 的 `config.yaml`。更换挂载根目录时更新该变量并重建容器；只切换根下场景目录时直接在页面填写并加载即可。
...
```text
hub.noematrix.cn/pharmacy/knowledge_shelf_query_runtime:v1.2.0
```
